...
// because the surface is shaded here rather than interpolated from the
// vertices: the injected surface body is compiled into this stage too, and asked
// how it faces and looks at this pixel's own crude point. Shading detail
...
void main() {
   ...
   CdlodSurfaceShading shading = cdlodSurfaceShading(crudePoint, dFdx(vert_crudeOffset),
                                                     dFdy(vert_crudeOffset));
   vec3 viewNormal = normalize(vert_bodyToView * shading.normal);

   if (vert_color.a < 1.0 / 255.0) discard;

   // The surface tints the body rather than replacing it: white leaves the body
