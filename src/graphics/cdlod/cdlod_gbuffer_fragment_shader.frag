...
in vec3 vert_crudePoint;
flat in mat3 vert_bodyToView;
in vec4 vert_color;
flat in float vert_emissiveScalar;

void main() {
   // Affine in the patch coordinate, so what arrives is exactly the point the
   // surface would be asked for at this pixel.
   vec3 viewNormal = normalize(vert_bodyToView * cdlodSurfaceNormal(vert_crudePoint));
